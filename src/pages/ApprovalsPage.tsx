...
import { Eye, Check, X, Bell, ChevronRight, Clock, CheckCircle, XCircle, AlertCircle, ChevronDown, Sparkles, Trash2 } from "lucide-react";
import { PageHero } from "@/components/ui/page-hero";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { addOperationLog } from "@/lib/operationsLogStore";
import { getDeletionRequests, updateDeletionRequest, getDeletionMatrices, formatAssignee, type DeletionRequest, type DeletionMatrix } from "@/lib/matrixStore";
...
const initialRequests: ApprovalRequest[] = [
  {
    id: "1", kpiCode: "KPI-2026-001", kpiName: "Aylıq Satış Hədəfi", kpiType: "Absolut Hədəf",
    createdDate: "10.04.2026", createdBy: "Əli Həsənov", kpiOwner: "Samir Həsənov",
    department: "Satış Departamenti", currentStep: 1, status: "pending", target: "5M AZN",
    description: "Aylıq satış hədəfinin yerinə yetirilməsi üçün əsas göstərici.",
    approvalChain: [
      { role: "Şöbə Müdiri", person: "Kamran Quliyev", status: "approved", date: "11.04.2026", comment: "Hədəf uyğundur.", createdDate: "10.04.2026" },
      { role: "Departament Direktoru", person: "Cari İstifadəçi", status: "pending", createdDate: "10.04.2026" },
      { role: "Kurator", person: "Nigar Hüseynova", status: "waiting", createdDate: "10.04.2026" },
      { role: "HR", person: "Günel Əlizadə", status: "waiting", createdDate: "10.04.2026" },
    ],
  },
  {
    id: "2", kpiCode: "KPI-2026-005", kpiName: "Müştəri Məmnuniyyəti", kpiType: "Faiz Hədəfi",
    createdDate: "08.04.2026", createdBy: "Leyla Məmmədova", kpiOwner: "Leyla Həsənova",
    department: "Müştəri Xidmətləri", currentStep: 0, status: "pending", target: "90%",
    description: "Müştəri məmnuniyyət indeksinin hədəf səviyyəyə çatdırılması.",
    approvalChain: [
      { role: "Şöbə Müdiri", person: "Cari İstifadəçi", status: "pending", createdDate: "08.04.2026" },
      { role: "Departament Direktoru", person: "Farid Həsənov", status: "waiting", createdDate: "08.04.2026" },
      { role: "HR", person: "Günel Əlizadə", status: "waiting", createdDate: "08.04.2026" },
    ],
  },
  {
    id: "3", kpiCode: "KPI-2026-003", kpiName: "İnnovasiya İndeksi", kpiType: "Benchmark",
    createdDate: "05.04.2026", createdBy: "Rəşad Əliyev", kpiOwner: "Rəşad Əliyev",
    department: "R&D", currentStep: 3, status: "approved", target: "80%",
    description: "İnnovasiya və yenilikçilik indeksinin ölçülməsi.",
    approvalChain: [
      { role: "Şöbə Müdiri", person: "Kamran Quliyev", status: "approved", date: "06.04.2026", comment: "Yaxşı planlaşdırılıb.", createdDate: "05.04.2026" },
      { role: "Departament Direktoru", person: "Farid Həsənov", status: "approved", date: "07.04.2026", comment: "Razıyam.", createdDate: "05.04.2026" },
      { role: "Kurator", person: "Nigar Hüseynova", status: "approved", date: "08.04.2026", createdDate: "05.04.2026" },
      { role: "HR", person: "Günel Əlizadə", status: "approved", date: "09.04.2026", comment: "Son təsdiq.", createdDate: "05.04.2026" },
    ],
  },
  {
    ...
    approvalChain: [
      { role: "Şöbə Müdiri", person: "Kamran Quliyev", status: "approved", date: "04.04.2026", comment: "Təsdiqləndi.", createdDate: "03.04.2026" },
      ...
      { role: "Kurator", person: "Nigar Hüseynova", status: "waiting", createdDate: "03.04.2026" },
    ],
  },
  {
    ...
    approvalChain: [
      { role: "Şöbə Müdiri", person: "Cari İstifadəçi", status: "pending", createdDate: "12.04.2026" },
...
const ApprovalsPage = () => {
  ...
  const [notifications, setNotifications] = useState<{ id: string; message: string; date: string; read: boolean }[]>([
    { id: "n1", message: "KPI-2026-001 təsdiqiniz gözləyir", date: "10.04.2026", read: false },
    { id: "n2", message: "KPI-2026-005 təsdiqiniz gözləyir", date: "08.04.2026", read: false },
    { id: "n3", message: "KPI-2026-006 təsdiqiniz gözləyir", date: "12.04.2026", read: false },
  ]);
  const [showNotifications, setShowNotifications] = useState(false);

  // ---- Deletion requests live from store ----
  const [deletionReqs, setDeletionReqs] = useState<DeletionRequest[]>([]);
  const [deletionMatrices, setDeletionMatrices] = useState<DeletionMatrix[]>([]);
  useEffect(() => {
    const reload = () => {
      setDeletionReqs(getDeletionRequests());
      setDeletionMatrices(getDeletionMatrices());
    };
    reload();
    window.addEventListener("matrix:updated", reload);
    window.addEventListener("storage", reload);
    return () => {
      window.removeEventListener("matrix:updated", reload);
      window.removeEventListener("storage", reload);
    };
  }, []);

  const deletionApprover = useMemo(() => {
    const m = deletionMatrices[0];
    if (!m?.approver) return { role: "Silinmə Təsdiqləyicisi", person: "—" };
    return {
      role: m.approver.type === "role" ? m.approver.name : "Silinmə Təsdiqləyicisi",
      person: m.mode === "position" && m.approver.type === "role" ? m.approver.name : formatAssignee(m.approver),
    };
  }, [deletionMatrices]);

  const deletionAsRequests: ApprovalRequest[] = useMemo(() =>
    deletionReqs.map((r): ApprovalRequest => {
      const date = new Date(r.requestedAt).toLocaleDateString("az-AZ");
      return {
        id: `del-${r.id}`,
        kpiCode: `DEL-${r.kpiId}`,
        kpiName: r.kpiName,
        kpiType: "KPI Silinmə Sorğusu",
        createdDate: date,
        createdBy: r.requestedBy,
        kpiOwner: r.requestedBy,
        department: "—",
        currentStep: 0,
        status: r.status,
        target: "—",
        description: `"${r.kpiName}" KPI kartının silinməsi üçün sorğu. Silinmə matrisi üzrə təsdiq tələb olunur.`,
        approvalChain: [{
          role: deletionApprover.role,
          person: r.status === "pending" ? "Cari İstifadəçi" : deletionApprover.person,
          status: r.status === "pending" ? "pending" : (r.status as any),
          date: r.status !== "pending" ? date : undefined,
          createdDate: date,
        }],
      };
    }), [deletionReqs, deletionApprover]);

  const allRequests = useMemo(() => [...requests, ...deletionAsRequests], [requests, deletionAsRequests]);

  const pendingRequests = allRequests.filter(r => r.status === "pending" && r.approvalChain[r.currentStep]?.person === "Cari İstifadəçi");
  const approvedRequests = allRequests.filter(r => r.status === "approved");
  const rejectedRequests = allRequests.filter(r => r.status === "rejected");

  const handleApprove = (req: ApprovalRequest) => {
    if (req.id.startsWith("del-")) {
      const realId = req.id.slice(4);
      updateDeletionRequest(realId, "approved");
      addOperationLog({ kpiName: req.kpiName, team: "—", period: "—", status: "deleted" });
      setNotifications(prev => [...prev, {
        id: `n-${Date.now()}`,
        message: `${req.kpiCode} silinmə sorğusu təsdiqləndi`,
    ...
    setRequests(prev => prev.map(r => {
      ...
      if (!isLast) {
        chain[nextStep] = { ...chain[nextStep], status: "pending" };
        setNotifications(prev => [...prev, {
          id: `n-${Date.now()}`,
          message: `${r.kpiCode} - ${r.kpiName} təsdiqi ${chain[nextStep].person} tərəfindən gözləyir`,
          date: new Date().toLocaleDateString("az-AZ"),
          read: false,
        }]);
      } else {
        setNotifications(prev => [...prev,
          { id: `n-${Date.now()}a`, message: `${r.kpiCode} - ${r.kpiName} tam təsdiq edildi!`, date: new Date().toLocaleDateString("az-AZ"), read: false },
        ]);
        addOperationLog({ kpiName: r.kpiName, team: r.department, period: "—", status: "approved" });
      }
  ...
  const handleReject = (req: ApprovalRequest) => {
    ...
    if (req.id.startsWith("del-")) {
      const realId = req.id.slice(4);
      updateDeletionRequest(realId, "rejected");
      setNotifications(prev => [...prev, {
        id: `n-${Date.now()}`,
        message: `${req.kpiCode} silinmə sorğusu imtina edildi. Səbəb: ${rejectReason}`,
        date: new Date().toLocaleDateString("az-AZ"),
        read: false,
      }]);
      setRejectReason("");
    ...
    setRequests(prev => prev.map(r => {
      ...
      setNotifications(prev => [...prev, {
        id: `n-${Date.now()}`,
        message: `${r.kpiCode} - ${r.kpiName} imtina edildi. Səbəb: ${rejectReason}`,
        date: new Date().toLocaleDateString("az-AZ"),
        read: false,
      }]);
  ...
  const getCurrentApprover = (req: ApprovalRequest) => {
    if (req.status === "approved") return "Tamamlandı";
    if (req.status === "rejected") return "İmtina edilib";
    const step = req.approvalChain[req.currentStep];
    return step ? `${step.role} - ${step.person}` : "Tamamlandı";
  };
          ...
          {selectedRequest && (
            <>
              ...
              <div className="space-y-4">
                      ...
                      <div key={i}>
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                            step.status === "approved" ? "bg-zone-green-bg text-zone-green-text" :
                            step.status === "rejected" ? "bg-zone-red-bg text-zone-red-text" :
                            step.status === "pending" ? "bg-zone-yellow-bg text-zone-yellow-text" :
                          ...
                          <button
                            ...
                            className="w-6 h-6 rounded flex items-center justify-center hover:bg-secondary transition-colors shrink-0"
                            title="Mərhələ detalları"
                          >
                        ...
                        {selectedStepIndex === i && (
                          <div className="ml-11 mt-2 p-3 bg-secondary rounded-lg border border-border space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-xs">
                              ...
                              <div><span className="text-muted-foreground">Status:</span> <span className={`font-medium ${
                                step.status === "approved" ? "text-zone-green-text" :
                                step.status === "rejected" ? "text-zone-red-text" :
                                step.status === "pending" ? "text-zone-yellow-text" : "text-muted-foreground"
                              }`}>{step.status === "approved" ? "Təsdiq edilib" : step.status === "rejected" ? "İmtina edilib" : step.status === "pending" ? "Gözləyir" : "Növbədə"}</span></div>
                            ...
                            {step.status === "pending" && step.person === "Cari İstifadəçi" && (
                ...
                {/* Action buttons */}
                {selectedRequest.status === "pending" && selectedRequest.approvalChain[selectedRequest.currentStep]?.person === "Cari İstifadəçi" && (
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <textarea
                            ...
                            rows={2}
                            className="w-full mt-1 px-3 py-2.5 text-sm border border-border rounded-lg bg-background resize-none focus:ring-2 focus:ring-ring"
                          />
                        </div>
