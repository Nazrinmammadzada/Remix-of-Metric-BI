...
FROM (
  ...
  FROM (
    SELECT
      ...
      row_number() OVER (
        PARTITION BY oc_inner.id, concat(COALESCE(elem->>'cardId', ''), '::', COALESCE(elem->>'assigneeId', lower(trim(split_part(COALESCE(elem->>'assigneeName', ''), '—', 1)))))
        ORDER BY CASE WHEN elem->>'status' = 'completed' THEN 1 ELSE 0 END DESC, COALESCE(NULLIF(elem->>'updatedAt', '')::numeric, 0) DESC
      ) AS rn
...
WHERE oc.id = deduped.id AND oc.catalog_key = 'kpi_set_entries';
    ...
    SELECT
      ...
      row_number() OVER (
        PARTITION BY oc_inner.id,
          concat(COALESCE(elem->>'parentId', 'root'), '::', COALESCE(elem->>'rootId', ''), '::', lower(trim(COALESCE(elem->>'cardName', ''))), '::', lower(trim(COALESCE(elem->>'goalName', ''))), '::', COALESCE(elem->>'assigneeId', lower(trim(COALESCE(elem->>'assigneeName', '')))))
...
WHERE oc.id = deduped.id AND oc.catalog_key = 'cascade_tree';